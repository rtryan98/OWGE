...
void cs_main(uint3 id : SV_DispatchThreadID)
{
    ...
    float mag = max(0.000001, length(wavenumber));
    float2 spectrum_minus_k = complex_conjugate((bnd.spectrum_tex.load_2d<float4>(uint2(bnd.size, bnd.size) - id.xy) % bnd.size).xy);
    float omega_k = bnd.angular_frequency_tex.load_2d<float>(id.xy);

    float phi = bnd.time * omega_k; // omega(k) * t in [Tessendorf 2001]
    float2 cmul_term = complex_from_polar(1.0, phi);
    float2 developed_spectrum = 0.5 * (complex_mul(spectrum, cmul_term) + complex_mul(spectrum_minus_k, float2(cmul_term.x, -cmul_term.y)));
    ...
    bnd.developed_spectrum_tex.store_2d<float2>(id.xy, packed_spectrum_x_y);
}
